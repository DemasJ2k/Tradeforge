...
export default function AgentPanel() {
  ...
  const agentTypeLabel = (agent: Agent) => {
    const t = agent.risk_config?.agent_type;
    if (t === "scalping") return "Scalping";
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center justify-center py-10 text-center">
          ...
          <p className="text-xs text-muted-foreground mb-4 max-w-xs">
            Create your first ML-powered trading agent. Choose between Scalping or Expert mode.
          </p>
                ...
                <div className="rounded-lg border border-card-border bg-background/50 hover:border-accent/30 transition-colors overflow-hidden">
                      ...
                      <div className="flex items-center gap-3 min-w-0">
                        ...
                        <div className="min-w-0">
                          ...
                          <div className="text-xs text-muted-foreground truncate flex items-center gap-1">
                            {agent.risk_config?.agent_type === "scalping" ? (
                              <Badge className="bg-amber-500/20 text-amber-400 text-[9px] py-0 gap-0.5"><Zap className="h-2.5 w-2.5" />Scalping</Badge>
                            ) : agent.risk_config?.agent_type === "expert" ? (
                              <Badge className="bg-emerald-500/20 text-emerald-400 text-[9px] py-0 gap-0.5"><Brain className="h-2.5 w-2.5" />Expert</Badge>
                            ) : (
                              <Badge className="bg-zinc-500/20 text-zinc-400 text-[9px] py-0">Agent</Badge>
                            )}
                            <span>{agent.symbol} - {agent.timeframe}</span>
                            {agent.broker_name && (
                              <span className="capitalize text-accent/70">- {agent.broker_name}</span>
                            )}
                          </div>
                        </div>
                      </div>
                  ...
                  <CollapsibleContent>
                    <AgentInlineDetail
                      agent={agent}
                      detail={detail}
                      agentTypeLabel={agentTypeLabel(agent)}
      ...
      <Dialog open={!!editAgent} onOpenChange={(open) => { if (!open) setEditAgent(null); }}>
        <DialogContent className="max-w-md max-h-[90vh] overflow-y-auto">
          ...
          {editAgent && (
            ...
            {/* Agent Type badge */}
            {editAgent.risk_config?.agent_type && (
              <div className="flex items-center gap-2">
                <Label className="block text-xs text-muted-foreground">Agent Type</Label>
                <Badge className={editAgent.risk_config.agent_type === "scalping" ? "bg-amber-500/20 text-amber-400" : "bg-emerald-500/20 text-emerald-400"}>
                  {editAgent.risk_config.agent_type === "scalping" ? "Scalping" : "Expert"}
                </Badge>
              </div>
            )}
            <div>
