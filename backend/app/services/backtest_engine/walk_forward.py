"""
Walk-Forward Validation — V3 Engine.

Splits data into rolling or anchored windows, runs backtests on each
out-of-sample segment, and aggregates results.
...
def walk_forward_backtest(
    ...
    Args:
        ...
        train_pct:         % of each fold for training (default 70)
    ...
    running_balance = cfg.initial_balance
    oos_equity: list[float] = [running_balance]
    ...
    for w in windows:
        ...
        if test_result.equity_curve:
            start_eq = test_result.equity_curve[0] if test_result.equity_curve else running_balance
            for eq_val in test_result.equity_curve[1:]:
                delta = eq_val - start_eq
    ...
    if all_oos_trades:
        pnls = [t.get("pnl", 0) for t in all_oos_trades]
        ...
        result.oos_win_rate = round(len(wins) / len(pnls) * 100, 2)
        ...
        result.oos_profit_factor = round(
            ...
            999.0 if gross_profit > 0 else 0.0, 4
        )

        result.oos_avg_win = round(gross_profit / len(wins), 2) if wins else 0.0
        result.oos_avg_loss = round(gross_loss / len(losses), 2) if losses else 0.0
        result.oos_expectancy = round(result.oos_net_profit / len(pnls), 2)
        ...
        if len(pnls) > 1:
            ...
            result.oos_sharpe_ratio = round(
                (mean_p / std_p) * math.sqrt(252), 4
            ) if std_p > 0 else 0.0

    # Consistency: % of folds with positive net profit
    profitable = sum(1 for p in result.fold_net_profits if p > 0)
    result.consistency_score = round(
        profitable / n_folds * 100, 1
    ) if n_folds > 0 else 0.0

    logger.info(
        "Walk-Forward complete: %d folds, %d OOS trades, "
        "%.1f%% WR, PF %.2f, consistency %.0f%%",
...
def _run_segment(
    ...
    seg_cfg = EngineConfig(**{
        f.name: getattr(config, f.name)
        for f in config.__dataclass_fields__.values()
    })
    seg_cfg.initial_balance = balance
...
def _result_to_stats(result: BacktestResult) -> dict:
    """Convert BacktestResult to a summary dict."""
    return {
