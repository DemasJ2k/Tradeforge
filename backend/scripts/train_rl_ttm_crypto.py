"""
Train RL Agent on TTM Squeeze Signals for BTCUSD H1
====================================================
Custom PPO agent for BTC that combines:
  - Bollinger Band / Keltner Channel squeeze detection
  - Squeeze fire (transition from squeeze ON to OFF)
  - Momentum direction from midline (EMA20)

Designed for BTC's high-volatility breakout behavior after consolidation.

The agent sees:
  - Technical features (25 dims: returns, vol, RSI, BB/KC squeeze, momentum, etc.)
  - Position state (7 dims: position, PnL, drawdown, etc.)

Actions:
  0 = SKIP (reject signal)
  1 = TAKE (accept signal, direction from squeeze fire)
  2 = CLOSE (close current position)

Usage:
  python train_rl_ttm_crypto.py --timesteps 1000000
  python train_rl_ttm_crypto.py --timesteps 500000 --n-envs 2
"""
...
def compute_bb(closes, period=20, mult=2.0):
    """Bollinger Bands: returns (upper, mid, lower, bb_width)."""
    n = len(closes)
    ...
    return upper, mid, lower, bb_width


def compute_kc(closes, highs, lows, atr, period=20, mult=1.5):
    """Keltner Channels: returns (upper, mid, lower, kc_width)."""
    n = len(closes)
    mid = compute_ema(closes, period)
    upper = np.zeros(n)
    lower = np.zeros(n)
    kc_width = np.zeros(n)
    for i in range(period - 1, n):
        ...
        if mid[i] > 0:
            kc_width[i] = (upper[i] - lower[i]) / mid[i]
    return upper, mid, lower, kc_width


# -- Feature matrix ------------------------------------------------

def build_feature_matrix(bars, atr, wr, rsi,
                         bb_period=20, bb_mult=2.0,
                         kc_period=20, kc_mult=1.5):
    """Build 25-dim feature matrix (ttm_25) for each bar.
    ...
    Features (25 dims):
     ...
     14:  bb_kc_ratio   - BB width / KC width (squeeze tightness; < 1.0 = squeeze ON)
     15:  mom_midline   - (close - EMA20) / ATR (momentum from midline)
     16:  squeeze_on    - 1.0 if BB inside KC (squeeze ON), 0.0 if not
     ...
     24:  signal_type   - 1.0=squeeze fired + close > mid (long),
                          -1.0=squeeze fired + close < mid (short), 0=no signal
    """
    n = len(bars)
    ...
    # Pre-compute BB and KC
    bb_upper, bb_mid, bb_lower, bb_width = compute_bb(closes, bb_period, bb_mult)
    kc_upper, kc_mid, kc_lower, kc_width = compute_kc(
        closes, highs_arr, lows_arr, atr, kc_period, kc_mult
    )

    # EMA20 for midline reference
    ema20 = compute_ema(closes, 20)

    # Pre-compute squeeze state per bar
    squeeze_on_arr = np.zeros(n, dtype=np.float32)
    for i in range(max(bb_period, kc_period) - 1, n):
        if bb_upper[i] < kc_upper[i] and bb_lower[i] > kc_lower[i]:
            squeeze_on_arr[i] = 1.0

    # Cooldown tracker for signal generation
    cooldown_counter = 0
    ...
    for i in range(30, n):
        ...
        # Feature 14: BB width / KC width (squeeze tightness ratio)
        if kc_width[i] > 0:
            features[i, 14] = bb_width[i] / kc_width[i]
        else:
            features[i, 14] = 1.0

        # Feature 15: (close - EMA20) / ATR (momentum from midline)
        if atr[i] > 0:
            features[i, 15] = (c - ema20[i]) / atr[i]

        # Feature 16: Squeeze ON (BB inside KC)
        features[i, 16] = squeeze_on_arr[i]
        ...
        # Feature 24: Signal type (TTM Squeeze fire)
        # Squeeze fire = squeeze was ON previous bar, now OFF
        cooldown_counter = max(0, cooldown_counter - 1)
        if i >= 1 and squeeze_on_arr[i - 1] == 1.0 and squeeze_on_arr[i] == 0.0:
            # Squeeze just fired
            if cooldown_counter == 0:
                if c > ema20[i]:
                    features[i, 24] = 1.0   # Long signal
                else:
                    features[i, 24] = -1.0  # Short signal
...
# -- TTM Squeeze RL Environment ------------------------------------

def create_ttm_env(bars, features, atr, ema20,
                   spread=5.0, atr_sl_mult=2.0, atr_tp_mult=4.0):
    """Create RL env for BTC H1 TTM Squeeze signals."""
    import gymnasium as gym
    from gymnasium import spaces

    class BTCTTMSqueezeEnv(gym.Env):
        """
        RL env for BTC H1 TTM Squeeze signals.

        The agent is called ONLY when a squeeze fire signal occurs.
        Between signals, the env auto-advances and manages SL/TP.

        Key TTM Squeeze adaptations:
          - Squeeze detection via BB(20,2.0) inside KC(20,1.5)
          - Signal fires when squeeze transitions OFF
          - Direction from close vs EMA20 midline
          - 3-bar cooldown between signals
          - Max hold 200 bars (H1 = ~8 days)
          - $5 BTC spread
        """
        ...
        def _get_signal_direction(self, i):
            """Check if bar i has a TTM squeeze fire signal."""
        ...
        def _advance_to_signal(self):
            ...
            while self.current_bar < self.episode_end:
                ...
                if self.position_dir != 0:
                    ...
                    # Force close if held > 200 bars (H1 = ~8 days)
    ...
    return BTCTTMSqueezeEnv()


# -- Main training -------------------------------------------------

def main():
    parser = argparse.ArgumentParser(description="Train RL on BTC TTM Squeeze signals")
    ...
    parser.add_argument("--csv", default=r"D:\Doc\DATA\Backtest Data\BTCUSD_H1.csv")
    args = parser.parse_args()

    symbol = "BTCUSD"
    timeframe = "H1"
    model_prefix = "rl_ttm_btcusd_h1"
    ...
    print(f"\n{'=' * 60}")
    print(f"  Training RL Agent -- {symbol} {timeframe} (TTM Squeeze)")
    print(f"  Timesteps: {args.timesteps:,}")
    print(f"  BB({bb_period}, {bb_mult}) / KC({kc_period}, {kc_mult})")
    ...
    # Build features
    print("  Building feature matrix (ttm_25)...")
    features, ema20 = build_feature_matrix(
        bars, atr, wr, rsi,
    ...
    print(f"  TTM Squeeze fire signals: {n_long + n_short:,} (long={n_long:,}, short={n_short:,})")

    # Count squeeze bars
    n_squeeze = int(np.sum(features[:, 16] > 0))
    print(f"  Bars in squeeze: {n_squeeze:,} ({n_squeeze / len(bars) * 100:.1f}%)")

    if n_long + n_short == 0:
        print("\n  ERROR: No TTM Squeeze signals found! Check data and parameters.")
        return
    ...
    try:
        ...
        np.savez(
            ...
            obs_var=obs_rms.var,
            obs_count=np.array([obs_rms.count]),
        )
    ...
    results = {
        ...
        "strategy_id": "s46_ttm_squeeze_crypto",
        "timeframe": timeframe,
        "feature_space": "ttm_25",
    ...
    print(f"\n{'=' * 60}")
    print(f"  {symbol} {timeframe} TTM Squeeze RL training complete!")
    print(f"{'=' * 60}")
