"""
Train RL Agent on Larry Williams V2 (H1) -- NAS100 (US100)
============================================================
PPO agent trained to filter Williams breakout signals on NAS100 H1.
Uses V2 params: BF=0.5, SL=2.0x ATR, TP=6.0x ATR, SMA50 trend filter.

Same architecture as US30 H1 training, adapted for NAS100 data
and spread characteristics.

Usage:
  python train_rl_lw_nas100.py --timesteps 1000000
"""
...
def create_lw_h1_env(bars, features, atr, sma,
                      breakout_factor=0.5, spread=1.5,
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description="Train RL on Larry Williams V2 NAS100 (H1)")
    parser.add_argument("--timesteps", type=int, default=1000000)
    parser.add_argument("--n-envs", type=int, default=4)
    parser.add_argument("--output-dir", default=os.path.join(os.path.dirname(__file__), "..", "data", "ml_models"))
    args = parser.parse_args()

    csv_path = os.path.join(os.path.dirname(__file__), "..", "data", "uploads", "1772956365_US100_H1_50239bars.csv")
    print(f"\n{'=' * 60}")
    print(f"  Training RL Agent -- NAS100 H1 (Larry Williams V2)")
    ...
    checkpoint_dir = os.path.join(args.output_dir, "rl_lw_nas100_h1_checkpoints")
    ...
    # Save
    model_path = os.path.join(args.output_dir, "rl_lw_nas100_h1_final")
    model.save(model_path)
    env.save(os.path.join(args.output_dir, "rl_lw_nas100_h1_vecnorm.pkl"))
    ...
    try:
        ...
        onnx_path = os.path.join(args.output_dir, "rl_lw_nas100_h1.onnx")
        torch.onnx.export(
    ...
    results_path = os.path.join(args.output_dir, "rl_lw_nas100_h1_results.json")
    ...
    print(f"  {'=' * 50}")
    print(f"  Base V2:     PF ~1.10, WR ~28%, Target: improve signal quality")
